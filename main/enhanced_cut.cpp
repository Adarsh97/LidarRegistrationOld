  ...
  for( i = 0; i < total_points; i++)
	{
         ...
         if(cut==end)
         {
           ...
           KMeans kmeans(K, cut, total_attributes, max_iterations);
          	kmeans.run(pointcollection);
           std::vector<Point> updatedpoints = pointcollection;
           for (size_t j = 0; j < updatedpoints.size() ; ++j)
           {
             updatedcloud.points[j+county].x = updatedpoints[j].getValue(0);
             updatedcloud.points[j+county].y = updatedpoints[j].getValue(1);
             updatedcloud.points[j+county].z = updatedpoints[j].getValue(2);
           }
           cut=0;
           county+=updatedpoints.size();
